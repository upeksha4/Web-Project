...
<head>
    ...
    <title>:.: Thanks Card Creations :.:</title>
    ...
    <?php
      if( mysql_num_rows( $selectRes )==0 ){
        ...
        while( $row = mysql_fetch_assoc( $selectRes ) ){
          echo "<tr><td>{$row['sub_date']}</td><td>{$row['card_type']}</td><td>{$row['card_size']}</td>
          <td>{$row['card_cat']}</td><td>{$row['des']}</td><td>{$row['pay_amount']}</td></tr>\n";
        }
      }
    ?>
  </tbody>
